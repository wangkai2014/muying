...
class HongHaiZi:
    ...
    def _getGoodsBasic(self):
        ...
        sql="select id,site_id,group_name,category,goods_id,goods_name,goods_url from goods_list where group_name like '%寝居%'"
        return db.query(sql)
    ...
    def fetch_comment(self):
        ...
        for g in goods:
            try:
                if db.exists('goods_comments','goods_id',g['id'])==0:
                    self.goods_id=g['id']
                    self.goods_name=g['goods_name']
                    ...
                    print 'comments_exists,skipped '+str(g['id'])
            except Exception,e:
                print e
                continue
    ...
    def _insert_comment(self):
        # print str(g['id'])
        if db.exists1('goods_comments','com_time',self.reviewtime,'com_u_name',self.username)==0:
            try:
